...
int nieto( int fdHijo[2] )
{
	...
	printf("-N Hellow Nietooooooooooooooo\n");

	printf("-N Nieto [g_hijo = %d]\n", g_hijo );
	...
	if ( !fin )
	{
		printf("-N Error fopen pipeIIB.cfg");
		fclose ( fin );
	}
	...
	{
		printf("-N Error escribiendo del Nieto al hijo\n");
	}

	printf("-N Bye Bye Nietooooooooooooooo\n");
	...
	while ( 1 )
	{
		...
		else if ( numHRead == -1 )
		{
			...
			if (errno == EAGAIN) {
				...
				strcpy( outH, "-H Hijo:(pipe empty)\n");
				write( 1, outH, strlen(outH)+1 );
				sleep(1);
			}
			else
			{
				//perror("read");
				memset( outH, '\0', sizeof(outH) );
				strcpy( outH, "-H Hijo: Error reading pipe\n");
				write( 1, outH, strlen(outH)+1 );
				exit(4);
			}
			g_hijo = -1;
		...
		{
			memset(outH, '\0', sizeof(outH) );
			strcpy( outH, "-H Hijo: End of conversation\n");
			write( 1, outH, strlen(outH)+1 );
			// read link
			close(fdFather[0]);
			exit(0);
...
//			printf("-H HijoB [%s]\n", lineHR);
				printf("-N Hellow Hijooooooooooooooo\n");

	
				strcpy(lineHW, "H2N");
				if ( write(fdHijo[1], lineHW, strlen(lineHW)) != strlen(lineHW) )
				{
					printf("-H Error escribiendo del hijo al Nieto\n");
				}
				...
				{
					printf("-H Error escribiendo del hijo al Padre\n");
				}
...
//				CLOSE_ALL ( fdFather );
				...
				//exit (-1);
				//break;
				printf("-H Bye Bye Hijooooooooooooooo\n");
